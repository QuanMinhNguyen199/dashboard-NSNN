...
npm run preview
npm run acceptance   # 14 tiêu chí nghiệm thu qua trình duyệt (cần dev server đang chạy)
```
...
validation** trước khi tới UI ([`src/dashboard/providers.ts`](src/dashboard/providers.ts)).
...
[`src/dashboard/observations.ts`](src/dashboard/observations.ts):
...
```
src/
  dashboard/   catalog.ts       danh mục nguồn, 21 khoản, địa bàn, chỉ tiêu
               observations.ts  kho quan sát gốc + sumOf/listObservations
               selectors.ts     dẫn xuất KPI, trend, cơ cấu, xếp hạng, waterfall
               providers.ts     Mock/API/MCP + runtime validation + whitelist intent
               useResource.ts   hook có huỷ request và chặn phản hồi cũ ghi đè
  state/       DashboardState.tsx  nơi duy nhất sở hữu URL state
  ui/          Grid.tsx         lưới 12 cột + layout resolver
               primitives.tsx   Card, Bars, Change, Segmented, ResourceView, định dạng
               charts.tsx       xu hướng, waterfall, bảng thay thế
               FilterBar.tsx · LocationMap.tsx · RevenuePreviewDrawer.tsx
  tabs/        bốn workspace
  styles/      dashboard.css    toàn bộ hệ thống thị giác
```

### Lưới động

Span đi qua CSS custom property đã kiểm tra, **không** dựng tên class Tailwind bằng chuỗi
động. `resolveRow` trong [`src/ui/Grid.tsx`](src/ui/Grid.tsx) nở widget còn lại thành 12 cột
